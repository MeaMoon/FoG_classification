...
# %%
# Data segmentation
# Data are being segmented into frames which include 64 rows, thus reducing the overall size of the data set
...
train_reshaped = X_train.reshape(len(X_train),64, 9).astype('float32')
with open('train64_reshaped.txt', 'w') as outfile_train:
...
test_reshaped = X_test.reshape(len(X_test),64, 9).astype('float32')
with open('test64_reshaped.txt', 'w') as outfile_test:
...
np.savetxt("predicted_labels_64.txt", Y_oracle)
...
plot_model(model, to_file='model_plot_64.png', show_shapes=True, show_layer_names=True)
model.save("model_cnn_64.h5")
